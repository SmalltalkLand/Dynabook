        ...
        unsafe fn from_raw_c(
            ...
            match opcode {
                ...
                4 => {
                    let _args = ::std::slice::from_raw_parts(args, 2);
                    Ok(Request::SetActions {
                        dnd_actions: _args[0].u,
                        preferred_action: _args[1].u,
                    })
                }
                _ => return Err(()),
            }
        ...
        fn into_raw(self, sender_id: u32) -> Message {
            match self {
                ...
                Event::SourceActions { source_actions } => Message {
                    sender_id: sender_id,
                    opcode: 1,
                    args: smallvec![Argument::Uint(source_actions),],
                },
                Event::Action { dnd_action } => Message {
                    sender_id: sender_id,
                    opcode: 2,
                    args: smallvec![Argument::Uint(dnd_action),],
                },
            }
        }
        unsafe fn from_raw_c(
            ...
            F: FnOnce(u32, &mut [wl_argument]) -> T,
        {
            match self {
                ...
                Event::SourceActions { source_actions } => {
                    let mut _args_array: [wl_argument; 1] = unsafe { ::std::mem::zeroed() };
                    _args_array[0].u = source_actions;
                    f(1, &mut _args_array)
                }
                Event::Action { dnd_action } => {
                    let mut _args_array: [wl_argument; 1] = unsafe { ::std::mem::zeroed() };
                    _args_array[0].u = dnd_action;
                    f(2, &mut _args_array)
                }
            }
        }
